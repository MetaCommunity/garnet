...
;;; The Unix implemenation of Garnet runs on top of CLX (for Mac
;;; version see below).  If CLX was installed with your version of
;;; Lisp (probably true) then the following form will likely work.  If not,
;;; You probably need to define the variable below.
#-apple(require :clx)
...
;;; The following variable allows you to read read .lisp files with
;;; Unix style #\linefeed for #\newline instead of the Mac OS <=9
;;; style of #\return for #\newline.
;;; Setting this should be harmless (unless you have a Mac OS .lisp
;;; file with an explicit #\linefeed character in a comment) and will
;;; cause lisp not to loose when reading unix style source files.
;(setq ccl::*linefeed-equals-newline* t)

;;; The following variable means that opening source files in FRED will
;;; convert #\linefeed to #\return for greater visibility.
;;; Unfortunately, the effect appears to be transient.
#+ccl-5.0 (setq ccl::*do-unix-hack* t)
