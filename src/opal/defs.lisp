...
;      (setq *is-this-a-color-screen?* nil)))
  ...
  (let ((color-screen-types '(:pseudo-color :direct-color
					    :static-color
					    :true-color
					    :quickdraw)) 
	;; RGA added a quick patch here to take into
	;; account not X-windows implementations.
	;; Really should move this to GEM.
	(screen-type #-(and apple (not clx))
