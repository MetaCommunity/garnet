...
#+ccl-3
(defparameter *process-with-main-event-loop*
  (find "Initial" ccl:*all-processes* :key #'ccl:process-name)
  "Events are processed by the Initial process in MCL 3.0")
...
(defun Listener-Process-Broken-P ()
  (when *Process-With-Main-Event-Loop*
    ...
    #+ccl-3(not (process-active-p *process-with-main-event-loop*))
    #-(or ccl-3 allegro lucid lispworks) NIL
    ))
...
(defun Timer-Process-Main-Loop (inter time once)
  (loop
   ...
   #+ccl-3 (ccl:process-allow-schedule)
...
(defun launch-timer-process (inter time once)
  "This only works in Lucid, Allegro and LispWorks CL and MCL 3.0"
